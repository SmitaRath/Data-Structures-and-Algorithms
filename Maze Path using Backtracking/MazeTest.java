...
public class MazeTest extends JFrame implements GridColors {
    ...
    public void solve() {
        ...
        //method call to check if path exist in maze @SmitaRath
        ...
        //method call to check all possible path of maze @SmitaRath
        theGrid.recolor(PATH, NON_BACKGROUND);
        theGrid.recolor(TEMPORARY,NON_BACKGROUND);
        ArrayList<ArrayList<PairInt>> results = m.findAllMazePaths(0,0);
        System.out.println("All Possible Paths of Maze");
        for(ArrayList<PairInt> arr:results){
            System.out.println(arr.toString());
        }
        JOptionPane.showMessageDialog(null, results);

        //method call to check shortest path of maze @SmitaRath
        ArrayList<PairInt> path = m.findMazePathMin(0, 0);
        System.out.println("Shortest Path of Maze");
        if (path != null) {
